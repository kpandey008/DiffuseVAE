...
class ImageWriter(BasePredictionWriter):
    def __init__(
        ...
        # Setup dirs
        self.base_save_path = os.path.join(self.output_dir, str(self.n_steps))
        self.comp_save_path = os.path.join(self.base_save_path, "compare")
        self.img_save_path = os.path.join(self.base_save_path, "images")

        # Create dirs
        os.makedirs(self.img_save_path, exist_ok=True)
        if self.compare:
            os.makedirs(self.comp_save_path, exist_ok=True)
    ...
    def write_on_batch_end(
        ...
        save_as_images(
            ...
            file_name=os.path.join(
                self.img_save_path, f"output_form1_{rank}_{batch_idx}"
            ),
        )
